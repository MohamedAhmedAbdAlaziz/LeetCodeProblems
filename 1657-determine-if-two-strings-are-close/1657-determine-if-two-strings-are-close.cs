public class Solution {
                ...
                else if (dic.ContainsKey(word2[i]))
  {
            ...
            var tt = dic.OrderBy(x => x.Value).ToDictionary(x => x.Key, y => y.Value).Select(x=>x.Value).ToList();
            var tt2 = dic2.OrderBy(x => x.Value).ToDictionary(x => x.Key, y => y.Value).Select(x=>x.Value).ToList();
